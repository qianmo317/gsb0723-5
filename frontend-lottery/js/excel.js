...
export function parseParticipantsFromFile(file) {
...
export function exportWinnersToExcel(winners, prizes) {
    const XLSX = getXLSX();

    const prizeIds = Object.keys(winners);
    if (prizeIds.length === 0) {
        throw new Error('暂无中奖记录可导出');
    }
    ...
    return { fileName, recordCount: exportData.length - 1 };
}
